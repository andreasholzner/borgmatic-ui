...
std::shared_ptr<BackupConfig> BorgmaticManagerImpl::newBorgmaticConfig() {
  auto newConfig = std::make_shared<BackupConfigImpl<BorgmaticBackupWorker>>();
...
std::vector<std::shared_ptr<BackupConfig>> BorgmaticManagerImpl::configs() {
  ...
  std::transform(configs_.begin(), configs_.end(), res.begin(), [](std::shared_ptr<BackupConfigImpl<BorgmaticBackupWorker>> c) {
